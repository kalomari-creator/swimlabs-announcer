...
function getScheduleDir(location) {
  return path.join(SCHEDULE_DIR, sanitizeDirSegment(location?.name || "Unknown"));
}

function getLocationFileTag(location) {
  return sanitizeDirSegment(location?.name || "Unknown").replace(/\s+/g, "_");
}
...
app.get("/api/guard-rotation", (req, res) => {
  try {
    ...
    if (!fs.existsSync(schedDir)) fs.mkdirSync(schedDir, { recursive: true });
    const filename = `guard_rotation_${getLocationFileTag(location)}_${date}_${type}.json`;
...
app.post("/api/guard-rotation", (req, res) => {
  try {
    ...
    const schedDir = getScheduleDir(location);
    if (!fs.existsSync(schedDir)) fs.mkdirSync(schedDir, { recursive: true });
    const filename = `guard_rotation_${getLocationFileTag(location)}_${rotationDate}_${rotationType}.json`;
    const filepath = path.join(schedDir, filename);
